...
function renderSummary(code, map, assets, ext, outDir, buildTime) {
  ...
  for (const asset of Object.keys(assets)) {
    ...
    const assetSize = Math.round(
        (assetSource.byteLength || Buffer.byteLength(assetSource, "utf8")) /
        1024);
  ...
  const orderedAssets = Object.keys(assets).sort(
      (a, b) => assetSizes[a] > assetSizes[b] ? 1 : -1);

  const sizePadding = totalSize.toString().length;

  let indexRender = `${codeSize.toString().padStart(sizePadding, " ")}kB  ${
      outDir}index${ext}`;
  let indexMapRender =
      map ? `${mapSize.toString().padStart(sizePadding, " ")}kB  ${
  ...
  for (const asset of orderedAssets) {
    ...
    output += `${assetSizes[asset].toString().padStart(sizePadding, " ")}kB  ${
        outDir}${asset}`;
  }
...
async function runCmd(argv, stdout, stderr) {
  ...
  case "run":
    ...
    outDir =
        resolve(require("os").tmpdir(), crypto.createHash('md5')
  ...
  case "build":
    ...
    const ncc = require("./index.js")(buildFile, {
      debugLog : args["--debug"],
      minify : args["--minify"],
    ...
    async function handler({err, code, map, assets, symlinks, stats}) {
      ...
      await Promise.all(
          (await new Promise(
              ...
              .map(file => new Promise(
                       (resolve, reject) => unlink(
                           file, err => err ? reject(err) : resolve()))));
      writeFileSync(`${outDir}/index${ext}`, code,
                    {mode : code.match(shebangRegEx) ? 0o777 : 0o666});
      if (map)
        writeFileSync(`${outDir}/index${ext}.map`, map);

      for (const asset of Object.keys(assets)) {
        const assetPath = outDir + "/" + asset;
        mkdirp.sync(dirname(assetPath));
        writeFileSync(assetPath, assets[asset].source,
        ...
        do {
          ...
          if (existsSync(nodeModulesDir))
            break;
                     ...
                     resolve(nodeModulesDir, "../../node_modules"));
        if (nodeModulesDir)
